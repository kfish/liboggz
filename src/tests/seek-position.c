...
off_t
test_seek_to_packet (OGGZ * reader, long n, int i)
{
  char buf[64];
  off_t offset, nread;
  oggz_position * pos;

  snprintf (buf, 64, "+ Seeking to packet index %d", i);
  INFO (buf);
  ...
  pos = &positions[i];
#ifdef DEBUG
  printf ("  Position: begin_page 0x%llx\tend_page 0x%llx\tpages %d\tsegment %d\n",
          pos->begin_page_offset, pos->end_page_offset, pos->pages, pos->begin_segment_index);
#endif
  
  if ((offset = oggz_seek_position (reader, pos)) == -1)
    FAIL ("oggz_seek_position()");

  read_called = 0;

#ifdef DEBUG
  printf ("%s: seek_position returned, offset 0x%lx\n", __func__, offset);
#endif

  while ((nread = oggz_read (reader, n-offset)) > 0) {
    offset += nread;
  }
...
seek_test (OGGZ * reader, long n)
{
  ...
  test_seek_to_packet (reader, n, 3);
  test_seek_to_packet (reader, n, 0);
  test_seek_to_packet (reader, n, 7);
  test_seek_to_packet (reader, n, MAX_PACKET-1);
  test_seek_to_packet (reader, n, MAX_PACKET-8);
  test_seek_to_packet (reader, n, MAX_PACKET-23);
  test_seek_to_packet (reader, n, 5);

  for (i=MAX_PACKET-1; i>=0; i--)
    test_seek_to_packet (reader, n, i);
...
main (int argc, char * argv[])
{
  ...
  INFO ("Testing oggz_seek_position()");
